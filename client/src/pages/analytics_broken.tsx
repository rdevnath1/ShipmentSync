...
import { TrendingUp, Package, DollarSign, Users, Calendar, Download, Lightbulb, AlertTriangle, CheckCircle, Target, Award, TrendingDown } from "lucide-react";
...
export default function Analytics() {
  ...
  const { data: rateComparisonData } = useQuery({
    queryKey: ["/api/rate-comparisons", dateRange, startDate, endDate],
  });
  ...
  const averageSavings = rateComparisons.length > 0 
    ? totalSavings / rateComparisons.length 
    : 0;
  ...
  const carrierWinData = Object.entries(carrierWins).map(([carrier, wins]) => ({
    carrier: carrier === 'quikpik' ? 'Quikpik' : carrier.toUpperCase(),
    wins,
    percentage: ((wins as number / rateComparisons.length) * 100).toFixed(1),
    color: carrier === 'quikpik' ? '#10b981' : carrier === 'fedex' ? '#f59e0b' : '#3b82f6'
  }));
  ...
  function generateSavingsData(comparisons: any[]) {
    const dailyData: any = {};
    
    comparisons.forEach(comparison => {
      const date = new Date(comparison.timestamp).toISOString().split('T')[0];
      if (!dailyData[date]) {
        dailyData[date] = { date, savings: 0, comparisons: 0 };
      }
      dailyData[date].savings += parseFloat(comparison.savings || 0);
      dailyData[date].comparisons += 1;
    });
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="text-center p-4 bg-purple-50 dark:bg-purple-950/20 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">${averageSavings.toFixed(2)}</div>
        ...
        {/* Rate Shopping Charts */}
        {rateComparisons.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Carrier Win Rate */}
            <Card>
              <CardHeader>
                <CardTitle>Carrier Win Rate</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      fill="#8884d8"
                      dataKey="wins"
                      label={({ carrier, percentage }) => `${carrier}: ${percentage}%`}
                    >
                      {carrierWinData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value: any) => [`${value} wins`, 'Count']} />
                  </PieChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={generateSavingsData(rateComparisons)}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="date" />
                    <YAxis />
                    <Tooltip formatter={(value: any) => [`$${value.toFixed(2)}`, 'Savings']} />
                    <Line 
                      type="monotone" 
                      dataKey="savings" 
                      stroke="#10b981" 
