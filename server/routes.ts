...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ShipStation webhook endpoint for tracking redirects
  app.post("/api/shipstation/webhook", async (req, res) => {
    try {
      const xmlData = req.body.toString();
      console.log("Received ShipStation webhook:", xmlData);
      
      // Parse XML to extract tracking information
      // ShipStation sends XML in format: <ShipNotice><OrderNumber>...</OrderNumber><TrackingNumber>...</TrackingNumber></ShipNotice>
      const orderMatch = xmlData.match(/<OrderNumber>(.*?)<\/OrderNumber>/);
      const trackingMatch = xmlData.match(/<TrackingNumber>(.*?)<\/TrackingNumber>/);
      const carrierMatch = xmlData.match(/<Carrier>(.*?)<\/Carrier>/);
      
      if (orderMatch && trackingMatch) {
        const orderNumber = orderMatch[1];
        const trackingNumber = trackingMatch[1];
        const carrier = carrierMatch ? carrierMatch[1] : 'Unknown';
        
        console.log(`ShipStation webhook - Order: ${orderNumber}, Tracking: ${trackingNumber}, Carrier: ${carrier}`);
        ...
        console.error("Invalid ShipStation webhook format:", xmlData);
  ...
  // Tracking redirect endpoint - when customers click ShipStation links
  app.get("/api/shipstation/track/:trackingNumber", async (req, res) => {
    try {
      const trackingNumber = req.params.trackingNumber;
      console.log(`Tracking redirect request for: ${trackingNumber}`);
      
      // Convert GV to QP format if needed
      const formattedTracking = trackingNumber.replace(/^GV/g, 'QP');
      
      // Log tracking redirect for audit
      await storage.logAudit(
        0, // System user
        'tracking_redirect',
        'shipstation',
        { originalTracking: trackingNumber, formattedTracking }
      );
      
      // Redirect to Quikpik tracking page
      const trackingUrl = `/tracking?track=${formattedTracking}`;
      ...
      const redirectHtml = `
        ...
        <html>
        <head>
          <title>Redirecting to Package Tracking...</title>
          <meta http-equiv="refresh" content="0; url=${trackingUrl}">
          <style>
        ...
        <body>
          <h2>Redirecting to Package Tracking...</h2>
          <div class="loader"></div>
          <p>Tracking Number: ${formattedTracking}</p>
          <p>If you're not redirected automatically, <a href="${trackingUrl}">click here</a>.</p>
          <script>
            setTimeout(function() {
